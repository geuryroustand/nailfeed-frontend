...
function transformCollection(strapiCollection: StrapiCollection): Collection {
  ...
  const owner = ownerSource
    ? {
        ...
        username: ownerSource.username ?? ownerSource.attributes?.username ?? ownerSource.handle,
...
export class CollectionsService {
  ...
  static async updateCollection(documentId: string, data: UpdateCollectionData): Promise<Collection> {
    ...
    const response = await fetch(`${getStrapiUrl()}/api/collections/${documentId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  static async removePostsFromCollection(documentId: string, postIds: string[]): Promise<Collection> {
    ...
    const remainingPostIds = currentCollection.postIds.filter((id) => !normalizedRemovals.includes(id.toString()))
  ...
  // Share a collection
  static async shareCollection(collectionId: string, shareData: CreateCollectionShareData): Promise<CollectionShare> {
  ...
  static async generateShareLink(collectionId: string): Promise<string> {
    ...
    const response = await fetch(`${getStrapiUrl()}/api/collections/${collectionId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
