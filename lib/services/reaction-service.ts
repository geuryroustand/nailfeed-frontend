...
export class ReactionService {
  ...
  ): Promise<Record<ReactionType, number> | any> {
    try {
      console.log(
        "[v0] ReactionService.getReactionCounts called with postId:",
        postId
      );
  ...
  static async getUserReaction(
    ...
    try {
      console.log(
        "[v0] ReactionService.getUserReaction called with postId:",
        postId,
        "userId:",
        userId
      );
      ...
      const endpoint = `/api/likes?${queryString}`;
      console.log(
        "[v0] Making API call to check user reaction with query:",
        queryString
      );
  ...
  static async addReaction(
    ...
    try {
      ...
      if (existingReaction) {
        console.log(
          "[v0] User changing reaction from",
  ...
  private static async updateReaction(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          "[v0] Auth proxy PUT response not ok:",
          response.status,
          response.statusText,
          errorText
        );
      ...
      throw new Error(
        "Failed to update reaction: " + (error.message || "Unknown error")
      );
    }
  }
  ...
  private static async createReaction(
    ...
    try {
      ...
      throw new Error(
        "Failed to create reaction: " + (error.message || "Unknown error")
      );
    }
  }
  ...
  static async removeReaction(
    reactionDocumentId: string | number
  ): Promise<boolean> {
    try {
      console.log(
        "[v0] Removing reaction with documentId:",
        reactionDocumentId
      );
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(
          "[v0] Auth proxy delete response not ok:",
          response.status,
          response.statusText,
          errorText
        );
  ...
  static async getDetailedReactions(postId: number | string): Promise<any> {
    try {
      console.log(
        "[v0] ReactionService.getDetailedReactions called with postId:",
        postId
      );
