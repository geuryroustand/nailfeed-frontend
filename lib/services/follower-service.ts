...
export async function getFollowers(username: string, page = 1, pageSize = 10): Promise<FollowListResponse> {
  try {
    ...
    if (!followersResponse.ok) throw new Error(`API Error: ${followersResponse.status} ${followersResponse.statusText}`)
    ...
    const users = (followersData.data || [])
      .map((follow: any) => {
        ...
        if (followerData.profileImage) {
          ...
          if (imageUrl) profileImageUrl = imageUrl.startsWith("http") ? imageUrl : `${apiUrl}${imageUrl}`
        }
...
export async function getFollowing(username: string, page = 1, pageSize = 10): Promise<FollowListResponse> {
  try {
    ...
    if (!followingResponse.ok) throw new Error(`API Error: ${followingResponse.status} ${followingResponse.statusText}`)
