...
export async function getFollowers(
  ...
  try {
    ...
    const targetUser =
      userResponse[0] || (userResponse.data && userResponse.data[0]);
    ...
    const followerUsers: NetworkUser[] = otherUsers.map((u: any) => {
      ...
      if (u.profileImage) {
        const imageUrl =
          u.profileImage.url || u.profileImage?.data?.attributes?.url;
        if (imageUrl) profileImageUrl = imageUrl;
      }
      let thumbnailUrl: string | undefined;
      if (u.profileImage?.formats?.thumbnail?.url)
        thumbnailUrl = u.profileImage.formats.thumbnail.url;
