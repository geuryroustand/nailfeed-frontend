...
export async function getFollowers(
  ...
  try {
    ...
    console.log(
      `Fetching user with followers: ${apiUrl}/api/users?${userQuery}`
    );
    ...
    const publishedFollowers = followers.filter(
      (follow: any) => follow.publishedAt !== null
    );

    // Collect follower relationship IDs
    const followerRelationIds = publishedFollowers.map(
      (follow: any) => follow.id
    );
    ...
    const followersResponse = await fetch(
      `${apiUrl}/api/users?${followersQuery}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      try {
        ...
        if (meResponse.ok) {
          ...
          const currentUserFollowing = (meData.following || []).map(
            (follow: any) =>
              follow.id?.toString() || follow.following?.id?.toString() || ""
          );
    ...
    const paginatedUsers = followerUsers.slice(
      startIndex,
      startIndex + pageSize
    );
...
export async function getFollowing(
  ...
  try {
    ...
    console.log(
      `Fetching user with optimized following query: ${apiUrl}/api/users?${queryString}`
    );
    ...
    const publishedFollowing = following.filter(
      (follow: any) => follow.publishedAt !== null
    );
      ...
      if (followingId && !processedIds.has(followingId)) {
        ...
        try {
          ...
          const followingResponse = await fetch(
            `${apiUrl}/api/users?${followingQuery}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          ...
          if (followingResponse.ok) {
            ...
            const followingUser =
              followingData[0] || (followingData.data && followingData.data[0]);
            ...
            if (followingUser) {
              ...
              if (followingUser.profileImage?.formats?.thumbnail?.url) {
                thumbnailUrl = ensureAbsoluteUrl(
                  followingUser.profileImage.formats.thumbnail.url,
                  apiUrl
                );
              }
              ...
              followingUsers.push({
                id: followingUser.id,
                documentId:
                  followingUser.documentId || `user-${followingUser.id}`,
          ...
          console.error(
            `Error fetching following details for ID ${followingId}:`,
            error
          );
        }
      }
    ...
    const startIndex = (page - 1) * pageSize;
    const paginatedUsers = followingUsers.slice(
      startIndex,
      startIndex + pageSize
    );
  ...
  try {
    ...
    if (currentlyFollowing) {
      ...
      const unfollowResponse = await fetch(
        `${apiUrl}/api/users/unfollow/${targetUserId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!unfollowResponse.ok) {
        throw new Error(
          `API Error: ${unfollowResponse.status} ${unfollowResponse.statusText}`
        );
      }
      ...
      const followResponse = await fetch(
        `${apiUrl}/api/users/follow/${targetUserId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
