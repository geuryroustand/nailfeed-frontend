...
export class CommentsService {
  ...
  static async addComment(postId: string | number, documentId: string | undefined, content: string, threadOf?: number) {
    try {
      ...
      console.log("[v0] CommentsService.addComment - sending request:", {
        endpoint,
        body,
        postId,
        documentId,
      ...
      if (!response.ok) {
        const errorText = await response.text().catch(() => "")
        console.error("[v0] CommentsService.addComment - Error response:", errorText || `HTTP ${response.status}`)
        ...
        return {
          success: false,
          error: errorData.error?.message || errorText || `HTTP ${response.status}`,
          details: errorData,
        }
      }

      const result = await response.json()
      console.log("[v0] CommentsService.addComment - Success:", result)
      return result
    } catch (error) {
      console.error("[v0] CommentsService.addComment - Exception:", error)
      return {
        success: false,
