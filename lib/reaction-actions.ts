...
const API_BASE_URL =
  process.env.API_URL || "https://nailfeed-backend-production.up.railway.app";
...
export async function reactToPost(
  ...
  try {
    console.log(
      `Server Action: Adding reaction ${type} to post ${postId} by user ${userId}`
    );
    ...
    console.log(
      `Server Action: Checking existing reactions with URL: ${checkUrl}`
    );
    const checkResponse = await fetchWithAuth(checkUrl);

    if (!checkResponse.ok) {
      console.error(
        `Server Action: Failed to check existing reactions: ${checkResponse.status}`
      );
      console.error(`Server Action: Response:`, await checkResponse.text());
      throw new Error(
        `Failed to check existing reactions: ${checkResponse.status}`
      );
    }
    ...
    if (checkData.data && checkData.data.length > 0) {
      ...
      if (existingReaction.attributes.type === type) {
        ...
        if (!deleteResponse.ok) {
          console.error(
            `Server Action: Failed to remove reaction: ${deleteResponse.status}`
          );
          console.error(
          ...
          throw new Error(
            `Failed to remove reaction: ${deleteResponse.status}`
          );
        }
        ...
        if (!updateResponse.ok) {
          console.error(
            `Server Action: Failed to update reaction: ${updateResponse.status}`
          );
          console.error(
            `Server Action: Response:`,
          ...
          throw new Error(
            `Failed to update reaction: ${updateResponse.status}`
          );
        }
      ...
      if (!createResponse.ok) {
        console.error(
          `Server Action: Failed to create reaction: ${createResponse.status}`
        );
...
export async function getPostReactions(postId: number | string) {
  try {
    ...
    if (!response.ok) {
      console.error(
        `Server Action: Failed to get reactions: ${response.status}`
      );
        ...
        if (reactionsByType[type] !== undefined) {
          ...
          if (reaction.attributes.user && reaction.attributes.user.data) {
            ...
            reactionsByType[type].users.push({
              ...
              displayName:
                userData.attributes.displayName || userData.attributes.username,
    ...
    console.error(
      `Server Action: Error getting reactions for post ${postId}:`,
      error
    );
    return {
      success: false,
...
export async function getUserReaction(
  ...
  try {
    console.log(
      `Server Action: Getting user ${userId} reaction for post ${postId}`
    );
    ...
    if (!response.ok) {
      console.error(
        `Server Action: Failed to get user reaction: ${response.status}`
      );
      console.error(`Server Action: Response:`, await response.text());
    ...
    console.error(
      `Server Action: Error getting user reaction for post ${postId}:`,
      error
    );
    return {
      success: false,
