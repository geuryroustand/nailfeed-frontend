...
export async function toggleFollow(
  ...
  try {
    ...
    // Get the token from secure session
    ...
    const userResponse = await safeFetch(
      `${apiUrl}/api/users?filters[username][$eq]=${encodeURIComponent(
        username
      )}`,
      {
        headers: {
    ...
    const userDocumentId =
      userData.data?.[0]?.documentId || userData[0]?.documentId;
    ...
    const followCheckResponse = await safeFetch(
    ...
    const followDocumentId =
      followCheckData.data?.[0]?.documentId || followCheckData[0]?.documentId;

    console.log(
      `[v0] Current database follow state for ${username}: ${existingFollow}`
    );
    ...
    if (existingFollow) {
      ...
      console.log(
        `[v0] Sending DELETE request to unfollow ${username} with documentId ${followDocumentId}`
      );
...
export async function checkFollowStatus(username: string): Promise<{
  ...
  try {
    const apiUrl = getApiUrl();
    console.log(
      `Checking follow status for ${username} using API URL: ${apiUrl}`
    );
    ...
    if (!session || !session.strapiJWT) {
      console.log("No authentication session found, returning default values");
    ...
    try {
      ...
      const userUrl = `${apiUrl}/api/users?filters[username][$eq]=${encodeURIComponent(
        username
      )}`;
      ...
      const followsResponse = await safeFetch(
        `${apiUrl}/api/follows?${followsQuery}`,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      const isFollowing = !!(
        followsData.data?.length > 0 || followsData.length > 0
      );
