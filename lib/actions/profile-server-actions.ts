...
export async function checkFollowStatus(targetUsername: string) {
  try {
    ...
    const isFollowing = !!(
      followData.data?.length > 0 || followData.length > 0
    );
 ...
 */
export async function toggleFollowServer(
  username: string,
  ...
  try {
    ...
    const followers =
      updatedUserData.data?.attributes?.followers?.data ||
      updatedUserData.followers ||
      [];
