...
// Cache for the nail mask
let nailMaskCanvas: HTMLCanvasElement | null = null
...
async function loadMediaPipeScripts() {
  ...
  try {
    // Create script elements for MediaPipe dependencies
    const scripts = [
      {
...
// Load the nail mask image
async function loadNailMask(): Promise<HTMLCanvasElement> {
  if (nailMaskCanvas) return nailMaskCanvas
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (!ctx) {
        reject(new Error("Could not create canvas context for nail mask"))
        return
      }

      ctx.drawImage(img, 0, 0)
      nailMaskCanvas = canvas
      resolve(canvas)
    }
    img.onerror = () => reject(new Error("Failed to load nail mask"))
    img.src = "/nail-mask.png"
  })
}
...
export async function initMediaPipe() {
  try {
    if (typeof window !== "undefined") {
      ...
      // Check if window.Hands is available (loaded from CDN)
        ...
        try {
          ...
          handsModule = {
            ...
            onResults: (callback: Function) => {
              handsModule.resultsCallback = callback
            },
    ...
    // Even on error, create a fallback to allow the app to continue
    handsModule = {
      initialized: true,
      send: async (data: any) => {
        return { multiHandLandmarks: [] }
...
export async function detectHands(imageUrl: string): Promise<NailPosition[]> {
  try {
    ...
    // If we're using the fallback implementation, skip MediaPipe processing
    if (handsModule.initialized) {
      console.log("Using fallback nail detection (MediaPipe not fully available)")
      return fallbackNailDetection(imageUrl)
    }
...
// Extract the design from the original image using a nail-shaped mask
export async function extractNailDesign(designImageUrl: string): Promise<HTMLCanvasElement> {
  try {
    // Load the nail mask
    const maskCanvas = await loadNailMask()
    ...
    // Use the mask dimensions as the base size
    designCanvas.width = maskCanvas.width
    designCanvas.height = maskCanvas.height
    ...
    // We'll take the most visually interesting part (usually center)
    ...
    // Draw the design image scaled to fit the mask size
    ctx.drawImage(img, sx, sy, sw, sh, 0, 0, designCanvas.width, designCanvas.height)

    // Apply the mask using composition
    ...
    // Fallback to a simple circular extraction if mask fails
...
export async function applyNailDesign(sourceImageUrl: string, designImageUrl: string): Promise<string> {
  try {
    ...
    // Extract the nail design using the mask
