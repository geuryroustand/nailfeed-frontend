...
export async function getFollowers(username: string, page = 1, pageSize = 10): Promise<FollowListResponse> {
  try {
    ...
    if (!userDocumentId) {
      return { users: [], total: 0, page, pageSize, totalPages: 0, isAuthenticated }
    }
    ...
    return { users: [], total: 0, page, pageSize, totalPages: 0, isAuthenticated: false }
  }
}
...
export async function toggleFollowStatus(
  ...
  try {
    ...
    if (!token) {
      return { success: false, isFollowing: currentlyFollowing, message: "Authentication required to follow users" }
    }

    const apiUrl = getApiUrl()
    const targetDocId = await getUserDocumentIdByUsername(targetUsername)
    if (!targetDocId) {
      return { success: false, isFollowing: currentlyFollowing, message: "User not found" }
    }

    if (currentlyFollowing) {
      const followQuery = qs.stringify({
        filters: {
          $and: [{ follower: { documentId: { $eq: "me" } } }, { following: { documentId: { $eq: targetDocId } } }],
        },
      })
      ...
      if (!followResponse.ok) throw new Error(`API Error: ${followResponse.status} ${followResponse.statusText}`)
      const followJson = await followResponse.json()

      const followId = followJson.data?.[0]?.id || followJson[0]?.id
      if (!followId) return { success: true, isFollowing: false, message: "Already unfollowed" }

      const del = await fetch(`${apiUrl}/api/follows/${followId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      })
      if (!del.ok) throw new Error(`API Error: ${del.status} ${del.statusText}`)
      ...
      if (!create.ok) throw new Error(`API Error: ${create.status} ${create.statusText}`)
      revalidatePath(`/profile/${targetUsername}`)
