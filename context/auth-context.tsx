...
import { refreshAuthToken, fetchWithTokenRefresh, subscribeToTokenRefresh } from "@/lib/token-refresh"
...
interface AuthContextType {
  ...
  resetPassword: (code: string, password: string, passwordConfirmation: string) => Promise<boolean>
  updateProfile: (userData: Partial<User>) => Promise<User | null>
  uploadProfileImage: (file: File) => Promise<boolean>
  refreshToken: () => Promise<boolean>
  authenticatedFetch: (url: string, options?: RequestInit) => Promise<Response>
  handleSocialAuth: (response: AuthResponse) => Promise<boolean>
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Reset password
  const resetPassword = async (code: string, password: string, passwordConfirmation: string): Promise<boolean> => {
    setIsLoading(true)
    try {
  ...
  const uploadProfileImage = async (file: File): Promise<boolean> => {
    ...
    setIsLoading(true)
    try {
      const success = await AuthService.uploadProfileImage(token, user.id, file)
      if (success) {
  ...
  const handleAuthSuccess = (response: AuthResponse) => {
    ...
    AuthService.storeTokens(response.jwt)
  }

  const handleSocialAuth = async (response: AuthResponse): Promise<boolean> => {
    try {
      handleAuthSuccess(response)
      return true
    } catch (error) {
