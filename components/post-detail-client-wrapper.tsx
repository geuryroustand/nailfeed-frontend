...
export default function PostDetailClientWrapper({
  post,
  relatedPosts,
}: PostDetailClientWrapperProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-pink-50 border border-pink-200 rounded-lg p-4 mb-4 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <p className="text-sm text-gray-700">
              Log in to like, comment, and save posts to your collections.
            </p>
          </div>
          <Button
            size="sm"
              ...
              {post.tags && post.tags.length > 0
                ? post.description.split(" ").map((word, i) => {
                    const cleanWord = word
                      .toLowerCase()
                      .replace(/[^\w\s]/g, "");
                    const isTag = post.tags?.some(
                      (tag) => tag.toLowerCase() === cleanWord
                    );
                    return (
                      <span key={i}>
                        {isTag ? (
      ...
      <div
        id="comments-section"
        className="bg-white rounded-xl shadow-sm overflow-hidden mb-6 p-4 sm:p-6"
      >
