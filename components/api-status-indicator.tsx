...
export default function ApiStatusIndicator() {
  ...
  const checkApiStatus = async () => {
    ...
    try {
      ...
      // Try to fetch a simple endpoint that should be public
      const response = await fetch(`${baseUrl}/api/health-check`, {
        method: "GET",
        cache: "no-store",
      })

      // If health-check endpoint doesn't exist, try the root API
      if (!response.ok) {
        ...
        setStatus(rootResponse.ok ? "online" : "offline")
      } else {
        setStatus("online")
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === "production" && !process.env.NEXT_PUBLIC_VERCEL_ENV?.includes("preview")) {
      setIsVisible(false)
      return
    }
  ...
  return (
    <div className="fixed bottom-4 left-4 z-50">
      <div className="bg-white rounded-md shadow-md p-2 flex items-center space-x-2">
        {status === "loading" && <RefreshCw className="h-4 w-4 text-yellow-500 animate-spin" />}
        ...
        <button onClick={checkApiStatus} className="text-xs text-gray-500 hover:text-gray-700">
