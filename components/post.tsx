...
import { MessageCircle, MoreHorizontal, Trash2, AlertCircle, Heart, Edit, Flag, BookmarkPlus } from "lucide-react"
import Link from "next/link"
import { useRouter } from "next/navigation"
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu"
import type { BackgroundType } from "./post-background-selector"
import { toMediaItems, hasMedia, getPrimaryMedia, type PostWithMedia } from "@/lib/media-utils"
...
// Import QuickFollowButton
import { QuickFollowButton } from "@/components/post/quick-follow-button"
...
export default function Post({
  ...
  const [reaction, setReaction] = useState<Reaction>((post.userReaction as Reaction) || null)
  ...
  const [totalReactionCount, setTotalReactionCount] = useState<number>(post.likesCount || post.likes || 0)
  ...
  const [reactionCounts, setReactionCounts] = useState<Record<ReactionType, number>>(() => {
    const base = REACTION_TYPE_ORDER.reduce(
      (acc, type) => {
        acc[type] = 0
        return acc
      },
  ...
  const handleReactionSummaryUpdate = useCallback((counts: Record<ReactionType, number>, total: number) => {
  ...
  const handleReaction = async (reactionType: Reaction) => {
    ...
    try {
      ...
      const nextReaction: Reaction | null = reaction === reactionType ? null : reactionType
      setReaction(nextReaction)

      setReactionCounts((prev) => {
        const updated = { ...prev }

        if (previousReactionType && updated[previousReactionType as ReactionType] !== undefined) {
    ...
    switch (contentType) {
      case "text-background":
        if (post.background) {
          return (
            <div
              className={`rounded-lg p-6 mb-3 ${post.background.value} ${post.background.animation || ""}`}
        ...
        return (
          <div className="mb-3">
            <p className="text-sm">{formatDescriptionWithHashtags(post, routerInstance)}</p>
          </div>
        )
          ...
          <>
            ...
            {hasMedia(post) && (
              <div className="mb-3 w-full">
                ...
                >
                  <MediaGallery items={toMediaItems(post)} layout={post.galleryLayout || "grid"} maxHeight={400} />
                </div>
              </div>
            )}
          </>
  ...
  useEffect(() => {
    ...
    const newCount =
      post.commentsCount || post.comments_count || (Array.isArray(post.comments) ? post.comments.length : 0) || 0
  ...
  return (
    <>
      <div className={`bg-white rounded-xl shadow-sm overflow-hidden mb-4 mx-4 ${className}`}>
          ...
          <div className="flex items-center justify-between mb-3">
            <Link href={getProfileUrl(post)} className="flex items-center group">
              ...
              <div className="ml-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {isPostOwner() && (
                    <span className="text-xs bg-pink-100 text-pink-800 px-1.5 py-0.5 rounded-full">You</span>
                  )}
                  {!isPostOwner() && (
                    <QuickFollowButton
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {isPostOwner() && (
                  <DropdownMenuItem onClick={() => setEditModalOpen(true)} className="flex items-center">
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-1">
                ...
                {reactionSummary.filter((reaction) => reaction.count > 0).length > 3 && (
                  <span className="text-xs text-gray-500 ml-1">
                    +{reactionSummary.filter((reaction) => reaction.count > 0).length - 3} more
                  </span>
                )}
              </div>
        ...
        <div className="flex items-center justify-between py-1 relative">
          ...
          <SavePostButton
            postId={typeof post.id === "number" ? post.id : Number.parseInt(post.id.toString())}
